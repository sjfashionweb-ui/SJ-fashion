...
export function EditProductForm({ product, onClose, onUpdated }: Props) {
  ...
  // Images (Fallback to imageUrl if images array is empty)
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label className="text-amber-400 font-bold">Base Price (LKR)</Label>
                <Input
                  className="bg-neutral-950 border-amber-400/50 focus:border-amber-400"
                  type="number"
                  value={price}
            ...
            <div className="p-4 border border-white/5 rounded-xl bg-neutral-950/50">
              <div className="flex items-center gap-2 mb-3">
                ...
                <Label className="text-neutral-200 text-base">Product Images</Label>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 border border-amber-400/20 rounded-lg bg-amber-400/5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <Label className="text-amber-400 text-base">Bulk Pricing Tiers</Label>
                    <p className="text-xs text-neutral-400">Offer discounts for higher quantities.</p>
                  </div>
                </div>
                <Button type="button" size="sm" variant="outline" onClick={addTier} className="border-amber-400/30 text-amber-400 hover:bg-amber-400/20 h-8">
                ...
                {bulkPricing.map((tier, i) => (
                  <div key={i} className="flex items-end gap-3 p-3 border border-white/10 rounded-lg bg-neutral-950">
                    <div className="flex-1 space-y-1">
                      <Label className="text-xs text-neutral-400">{"If Qty is >="}</Label>
                    ...
                    <div className="flex-1 space-y-1">
                      <Label className="text-xs text-neutral-400">Price becomes (LKR)</Label>
                    ...
                    <Button type="button" size="icon" variant="ghost" onClick={() => removeTier(i)} className="h-8 w-8 hover:text-red-400 mb-0.5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label className="text-neutral-300">Variants (Size, Color, Stock)</Label>
                <Button type="button" size="sm" variant="outline" onClick={addVariant} className="border-white/10 hover:bg-white/5 h-8">
                ...
                {variants.map((v, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1fr_90px_auto] gap-2 items-end p-3 border border-white/10 rounded-lg bg-neutral-950">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input type="number" className="bg-neutral-900 border-white/10 h-8" value={v.stock} onChange={(e) => updateVariant(i, { stock: Number(e.target.value) })} />
                    </div>
                    <Button type="button" size="icon" variant="ghost" onClick={() => removeVariant(i)} className="h-8 w-8 hover:text-red-400 mb-0.5">
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                ))}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
              ...
              <Button type="submit" disabled={submitting} className="bg-amber-400 text-black hover:bg-amber-500 px-8 font-bold">
